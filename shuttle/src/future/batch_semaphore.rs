...
impl BatchSemaphore {
    ...
    pub fn close(&self) {
        if !ExecutionState::with(|s| s.in_cleanup()) {
            thread::switch(Event::batch_semaphore_rel(self.signature.clone()));
        }
    ...
    pub fn try_acquire(&self, num_permits: usize) -> Result<(), TryAcquireError> {
        thread::switch(Event::batch_semaphore_acq(self.signature.clone()));
...
impl Future for Acquire<'_> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        if !self.has_polled && (will_succeed || blocking_changes_state) {
            thread::switch(Event::batch_semaphore_acq(self.semaphore.signature.clone()));
        }
                ...
                match acquire_result {
                    ...
                    Err(TryAcquireError::NoPermits) => {
                        ...
                        *maybe_waker = Some(cx.waker().clone());
                        ...
                        ExecutionState::with(|s| {
                            s.current_mut().next_event = Event::batch_semaphore_acq(self.semaphore.signature.clone())
                        });
                        Poll::Pending
                    }
