...
impl<T: Copy + Eq> Atomic<T> {
    ...
    fn load(&self, order: Ordering) -> T {
        ...
        thread::switch(Event::atomic_read(self.signature.clone()));
        self.exhale_clock();
    ...
    fn store(&self, val: T, order: Ordering) {
        ...
        thread::switch(Event::atomic_write(self.signature.clone()));
        self.inhale_clock();
    ...
    fn swap(&self, mut val: T, order: Ordering) -> T {
        ...
        thread::switch(Event::atomic_read_write(self.signature.clone()));
